<?php
	...
	else if(isset($_POST['submit']))
	{
		...
		if(empty($Username) || empty($Password))
		{
			...
			header("location:../Example.php?msg=UserName and Password is required");exit;
		}
		else
		{
			...
			if(mysqli_num_rows($sql) > 0)
			{
				...
				$_SESSION['email_id']=$_POST['name'];
				// mysqli_query($db,"update OTP set email='$email' where id=1");
				...
				header('location:../controller/otp_send.php');exit;
			}
			else
			{
				header("location:../Example.php?msg=Username and Password is Wrong!");exit;
			}
		}
	}
	...
	function user()
	{
		...
		if(mysqli_num_rows($sql) > 0)
		{
			...
			if(mysqli_num_rows($ldate) > 0)
			{
				...
				// mysqli_query($db,"update OTP set email='$email' where id=2");

				$_SESSION['email_id']=$_POST['name'];
				header('location:../user/mail/otp_send.php');exit;
			}
			else
			{
